...
from src.lane_detection_pipeline.lane_detection_pipeline import (LaneDetectionPipeline,
                                                                 SavedPolynomialValues)
...
def run_full_pipeline(image):
    ...
    utils = GeneralUtils()
    calib_params = utils.read_yaml(
        "/home/charlie/repos/self_driving_car_nanodegree/lane_detection_node/src/lane_detection_pipeline/config/calibration_parameters.yaml"
    )
    img_params = utils.read_yaml(
        "/home/charlie/repos/self_driving_car_nanodegree/lane_detection_node/src/lane_detection_pipeline/config/image_processing_parameters.yaml"
    )
    lane_params = utils.read_yaml(
        "/home/charlie/repos/self_driving_car_nanodegree/lane_detection_node/src/lane_detection_pipeline/config/lane_detection_parameters.yaml"
    )
    ...
    processed_image = image_pipeline.combine_colour_gradient(
    ...
    if left_previous.detected or right_previous.detected == False:
        ...
        leftx_base, rightx_base, hist, out_img = lane_pipeline.histogram_image(
            processed_image, debug_img=False
        )
        ...
        rad_curve_l, rad_curve_r, curvature = lane_pipeline.find_radius_of_curvature(
            left_fit_m, right_fit_m, ploty
        )
        off_centre = lane_pipeline.find_position_off_centre(
            image_shape, left_fit_m, right_fit_m
        )

    else:
        left_fit, right_fit, left_fit_m, right_fit_m, left_fitx, right_fitx, ploty, result = lane_pipeline.search_around_poly(
            processed_image,
            left_previous.current_fit,
            right_previous.current_fit,
            debug_img=True,
        )
    ...
    if len(left_fit) and len(right_fit) > 0:
        ...
        if left_previous.bestx is not None:
            left_previous.bestx = (
                np.array(left_previous.bestx) + np.array(left_fitx)
            ) / 2.0
        else:
        ...
        if right_previous.bestx is not None:
            right_previous.bestx = (
                np.array(right_previous.bestx) + np.array(right_fitx)
            ) / 2.0
        else:
    ...
    cv2.putText(
        lane_overlay, rad_text, (40, 70), font, 1, (255, 255, 255), 2, cv2.LINE_AA
    )
    offset_text = "Ego Pose vs Centre Line: {:04.3f}".format(off_centre) + "(m)"
    cv2.putText(
        lane_overlay, offset_text, (40, 120), font, 1, (255, 255, 255), 2, cv2.LINE_AA
    )
...
def run_on_video(input_video=None, output_path=None):
    ...
    process_video.write_videofile(output_path, audio=False)


run_on_video(
    input_video="/home/charlie/repos/self_driving_car_nanodegree/lane_detection_node/test_videos/harder_challenge_video.mp4",
    output_path="/home/charlie/repos/self_driving_car_nanodegree/lane_detection_node/output_videos/harder_challenge_video_processed.mp4",
)
