...
class LaneDetectionPipeline:
    ...
    def determine_lane_pixels(self, binary_warped, leftx_base, rightx_base, debug_img=False, out_img=None):
        """
    ...
    def fit_poly(self, img_shape, leftx, lefty, rightx, righty, debug_img=False, out_img=None):
        """
        ...
        # In meters
        ym_per_pix = self.params["radius_of_curvature"]["ym_per_pix"]  # meters per pixel in y dimension
        xm_per_pix = self.params["radius_of_curvature"]["xm_per_pix"]  # meters per pixel in y dimension
        ...
        return left_fit, right_fit, left_fit_m, right_fit_m, left_fitx, right_fitx, ploty, out_img

    def search_around_poly(self, binary_warped, previous_left_fit, previous_right_fit, debug_img=False):
        """
        ...
        left_lane_inds = ((nonzerox > (previous_left_fit[0] * (nonzeroy ** 2) + previous_left_fit[1] * nonzeroy +
                                       previous_left_fit[2] - margin)) &
                          (nonzerox < (previous_left_fit[0] * (nonzeroy ** 2) +
        ...
        right_lane_inds = ((nonzerox > (previous_right_fit[0] * (nonzeroy ** 2) + previous_right_fit[1] * nonzeroy +
                                        previous_right_fit[2] - margin)) &
                           (nonzerox < (previous_right_fit[0] * (nonzeroy ** 2) +
        ...
        if debug_img:
            ...
            return new_left_fit, new_right_fit, new_left_m, new_right_m, left_fitx, right_fitx, ploty, result
        else:
            result = None
            return new_left_fit, new_right_fit, new_left_m, new_right_m, left_fitx, right_fitx, ploty, result
    ...
    def find_radius_of_curvature(self, left_fit_m, right_fit_m, ploty):
        ...
        left_curverad = ((1 + (
                    2 * left_fit_m[0] * y_eval * ym_per_pix + left_fit_m[1]) ** 2) ** 1.5) / np.absolute(
            2 * left_fit_m[0])
        right_curverad = ((1 + (
                    2 * right_fit_m[0] * y_eval * ym_per_pix + right_fit_m[1]) ** 2) ** 1.5) / np.absolute(
            2 * right_fit_m[0])
    ...
    def find_position_off_centre(self, image_shape, left_coeff_m, right_coeff_m):
        ...
        l_x_intercept = left_coeff_m[0] * y_to_m ** 2 + left_coeff_m[1] * y_to_m + left_coeff_m[2]
        r_x_intercept = right_coeff_m[0] * y_to_m ** 2 + right_coeff_m[1] * y_to_m + right_coeff_m[2]
...
def test_lane_pipeline(plot_hist=False, plot_sliding_windows=False, plot_poly_search=False):
    ...
    for image in images:
        ...
        # Histogram
        leftx_base, rightx_base, hist, out_img = process_lanes.histogram_image(combined_grad_colour, debug_img=False)
        ...
        leftx, lefty, rightx, righty, out_img = process_lanes.determine_lane_pixels(
            combined_grad_colour, leftx_base, rightx_base, debug_img=True, out_img=out_img
        )
        ...
        if plot_sliding_windows:
            ...
            cv2.imwrite(f"../../output_images/lane_detection/sliding_windows/{sliding_filename}", out_img)

        # Fit polynomial based on previous result
        new_left_fit, new_right_fit, new_left_m, new_right_m, left_fitx, right_fitx, ploty, result = \
            process_lanes.search_around_poly(combined_grad_colour, left_fit, right_fit, debug_img=True)

        if plot_poly_search:
            search_filename = image.split("/")[-1].split(".")[0]
            search_filename = search_filename + "_previous_search.jpg"
            cv2.imwrite(f"../../output_images/lane_detection/previous_polynomial_coefficients/{search_filename}", result)

        rad_curve_l, rad_curve_r, curvature = process_lanes.find_radius_of_curvature(new_left_m, new_right_m, ploty)
        off_centre = process_lanes.find_position_off_centre(image_shape, new_left_m, new_right_m)
        print(image)
